...
while read -r line; do
	...
	if [ ! -f ${WORKING_DIR}/bams/${INPUT_PREFIX}.${REF_CODE}.fds.bam ]; then
		printf "\n...submitting sample: $INPUT_DIR/$INPUT_PREFIX\n" | tee >(cat >&2)
		sbatch ${SCRIPTS_DIR}/fastq_align_each.sh $PROJECT $BATCH_NAME $GENOME $INPUT_DIR $INPUT_PREFIX $REF_CODE
	fi
done < $FASTQ_TABLE
